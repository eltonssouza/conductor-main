...
3. **Adopt the role(s)** — load the gate's role skill(s) with `$<skill-name>`
   (e.g. `$decide-architecture`) so you reason with that expert's lens. For
   parallel exploration you may explicitly spawn subagents; otherwise reason
   inline under the skill. Choose the roles `AGENTS.md` lists for the gate.
